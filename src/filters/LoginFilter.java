...
public class LoginFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        if (servletPath.matches("/css.*")
                ...
                ||(ForwardConst.ACT_TOP.getValue().equals(action) && ForwardConst.CMD_INDEX.getValue().equals(command))
                ||(ForwardConst.ACT_RSV.getValue().equals(action) && ForwardConst.CMD_TOP.getValue().equals(command))
                ) {
            // CSSフォルダ内は認証処理から除外する
            ...
            if (uv == null) {
                ...
                if (!(ForwardConst.ACT_AUTH.getValue().equals(action)
                    ...
                    //ログイン不要ページまたはログイン実行以外はトップページにリダイレクト
